...
public class ProductService {
	...
	/**
	 * 출고시에는 서비스단에서 재고량/출고량 비교 검사를 실시할 수 있고, 부정한 값이 입력된 경우 예외를 던져 controller에서 흐름을
	 * 분기하도록 한다.
	 * 
	 * @param pio
	 * @return
	...
	public int insertProductIO(ProductIO pio) {
		...
		if (PRODUCT_OUTPUT.equals(pio.getStatus())) {
			...
			if (p.getStock() < pio.getCount())
				throw new InsufficientOutputAmountException(
						"출고량 (" + pio.getCount() + ")이  재고량 (" + p.getStock() + ") 보다 많습니다.");
		}
		int result = 0;
		try {
